...
## 의존성 역전 원칙 ( `Dependency Inversion Priciple : D.I` )

객체 사이의 협력이 존재할 때,<br/>
본질을 담고 있는 것은 상위 수준의 정책이다.<br/>
다시 말해 "어떤 협력에서 중요한 정책이나 의사결정, 비즈니스의 본질을 담고 있는 것은 상위 수준의 클래스"라는 것이다.

의존성은 변경의 전파와 직접적인 연관이 있기 때문에<br/>
설계단에서 변경의 영향을 최소화하도록 의존성을 관리해야 한다.<br/>
그에 따라 상위 클래스는 어떤 식으로든 하위 수준의 클래스에 의존해선 안되는 것이다.

하위 수준의 변경으로 인해 상위 수준이 변경되는 문제를 해결하는 방법으로<br/>
어김없이 **추상화**가 등장한다.

추상화에 의존하도록 수정하면<br/>
하위 수준 클래스의 변경으로 인해 상위 수준의 클래스가 영향을 받는 것을 방지할 수 있고<br/>
더 나아가 상위 수준을 재사용할 때 하위 수준의 클래스에 얽매이지 않고도 다양한 재사용이 가능해진다.

(ex. `Movie` → `DiscountPolicy` ← `AmountDiscountPolicy`&`PercentDiscountPolicy` )

정리하자면 다음과 같다.
- 상위 수준의 모듈은 하위 수준의 모듈에 의존해서는 안된다. 둘 다 추상화에 의존해야 한다.
- 추상화는 구체적 사항에 의존해서는 안되며 반대로 구체적인 사항이 추상화에 의존하도록 해야 한다.

위와 같은 원칙을 **의존성 역전 원칙(D.I.P)** 라고 부른다.

<br/>

### D.I.P 와 패키지
위 예시로 사용한 `Movie`와 할인정책들을 다시 한 번 살펴보자.

패키지 구조는 <br/>
`Movie`를 가진 패키지와 추상 클래스 `DiscountPolicy`와 <br/>
구체화 클래스인 `AmountDiscountPolicy`&`PercentDiscountPolicy`를 가지는 패키지가 구성되어있다고 가정하고<br/>
이 패키지들을 이용해 모듈을 만들어 배포해야 한다.

하지만 배포를 해야 할 경우,<br/>
`Movie`를 다양한 컨텍스트에서 재사용하기 위해서는 불필요한 클래스들과 함께 배포될 것이다.<br/>
`Movie`는 `DiscountPolicy`에 의존하고 있기 때문에<br/>
배포 시엔 필요없을 `AmountDiscountPolicy`&`PercentDiscountPolicy`클래스까지 함께 배포된다는 것이다.

그렇게 되면 `DiscountPolicy`가 포함된 패키지 내 <u>어떤 클래스가 수정되더라도</u><br/> 
`Movie`의 패키지를 비롯하여 연관 패키지 전체가 재배포되는 상황을 피할 수 없다.

의존 관계가 여기서 끝나면 큰 문제 없겠지만<br/>
만약 `Movie`를 의존하거나 `Movie`가 의존하는 다른 클래스가 있다면 얘기가 달라진다.<br/>
의**존관계를 가지고 있는 또 다른 패키지의 클래스들까지 줄줄이 재배포**되는 상황이 연출된다.

따라서 추상화를 별도의 독립적인 패키지가 아닌<br/>
**클라이언트(`Movie`)가 속한 패키지**에 <u>**포함**</u>시켜야 한다.<br/>
또한 <u>재사용될 필요가 없는 클래스들</u>을 **별도의 독립적 패키지**에 모아야 한다.

이러한 기법에 대해 `SEPARATED INTERFACE 패턴`라고 부를 수 있다.

`Movie`와 추상 클래스 `DiscountPolicy`를 **하나의 패키지로 모음**으로서<br/>
`Movie`를 <u>특정한 컨텍스트로부터 완벽하게 **독립**</u> 시킬 수 있다.

즉, `Movie`를 <u>다른 컨텍스트에서 재사용</u>하기 위해서는<br/>
`AmountDiscountPolicy`&`PercentDiscountPolicy`클래스를 독립시킨 패키지 없이<br/>
**단지 두 클래스를 모은 패키지만 재사용**하면 되는 것이다.

정리하자면<br/>
의존성 역전 원칙에 따라 <u>상위 수준의 협력 흐름</u>을 **재사용하**기 위해서는<br/>
<u>추상화가 제공하는 **인터페이스의 소유권**</u> 역시 **역전**시켜야 한다.
