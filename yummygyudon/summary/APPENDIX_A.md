...
---
## "계약에 의한 설계" & "서브타이핑"
> ⭐️ 핵심 : "`클라이언트`와 `서버` 사이의 **견고한 협력**을 위해 <u>**준수해야 하는 규약**을 정의</u>하는 것"

- "`계약에 의한 설계`" → "**사전조건**(`클라이언트` 의무)" & "**사후조건**(`클라이언트` 기대)" 기술
- "`리스코프 치환 원칙`" → <u>**슈퍼 타입** 인스턴스 협력 `클라이언트`</u>의 관점 : **서브타입**의 인스턴스의 <u>슈퍼타입 **대체**</u>가 협력에 지장 없어야 한다.

즉, `서브타입`이 `L.S.P`에 만족시키기 위해서는 " `클라이언트`와 `슈퍼타입` 간에 체결된 **계약** "을 준수해야 한다.

<br/>

### 1. `계약` 규칙 : _Contract Rules_
> "협력에 참여하는 **객체**"에 대한 <u>기대 표현</u> 규칙<br/>
> : `슈퍼 타입` - `서브 타입` 사이의  ⌜ **사전조건** / **사후조건** / **불변식** ⌟ 에 대해 서술

- `서브 타입`에 <u>더 **강력**</u>한 **"사전 조건"** 정의 ❌ ( → _강화 가능 = 사후_ )
- `서브 타입`에 <u>더 **완화**</u>된 **"사후 조건"** 정의 ❌ ( → _완화 가능 = 사전_ )
- `슈퍼 타입`의 "**불변식**"은 `서브 타입`에서도 <u>반드시 **유지**</u>

<br/>

⁜ `사전조건` = 클라이언트의 **의무**<br/> : 
_클라이언트가 알 수 없는 범위인데 더 강력한 사전 조건이 있다면 클라이언트는 이를 지키는 것이 불가능 <br/>
→ **더 강력한 사전 조건 정의 금지**_

⁜ `사후조건` = 서버의 **의무** <br/> :
_즉, 결과값을 반환하는 <u>서브 타입 인스턴스</u>가 "**서버**의 역할"인데 잘못된 데이터를 반환하면 안됨<br/>
→ **더 완화된 사후 조건 정의 금지**_

⁜ `불변식` = ⌜ 메서드 실행 전 & 후 / 생성자 호출 ⌟ 때 모두 실행 <br/> : 
_서브타입은 슈퍼타입에 대체될 수 있어야 함.<br/>
→ **`서브 타입`에서도 <u>반드시 유지</u>**_

<br/>

### 2. `가변성` 규칙 : _Variance Rules_
> <u>교체 가능</u>한 **"타입"** 관련 규칙<br/>
> : ⌜ `Parameter` / `Return Type` ⌟의 "**변형**"과 관련된 규칙

- `서브 타입` "<u>메서드 **파라미터**</u>" : **"[반공변성](#반공변성--contravariance-)"** 필요
- `서브 타입` "<u>**리턴 타입**</u>" : **"[공변성](#공변성--covariance-)"** 필요
- `슈퍼 타입` "<u>**예외**</u>" : **"`슈퍼타입` 발생 예외와 <u>다른 타입 예외 ❌</u>"**

<br/>

⁜ 클라이언트 입장에서 <u>**협력의 결과**에 벗어나는</u> "서브타입에서 발생하는 숨겨진 예외" 발생<br/>
→ 슈퍼 타입으로의 대체가 불가능<br/>
→ **"`슈퍼타입` 발생 예외와 <u>다른 타입 예외 ❌</u>"**

<br/>

⁜ `슈퍼 타입` 대신 "`서브 타입`" **반환** <br/> :
_"<u>더 **강력**한 `사후 조건`</u>"을 정의하는 것과 동일 <br/><br/>
→ 클라이언트 입장에서 **대체 가능** <br/>
→ "<u>**리턴 타입**</u>" : **"[공변성](#공변성--covariance-)"** 필요_

> ✅ `리턴 타입 공변성 (Return Type Covariance)` <br/>: _부모 클래스에서 구현된 메서드를 자식 클래스에서 **오버라이딩** 할 때_,<br/>
> <u>부모 클래스에서 선언한 " **반환 타입**</u>의 `서브타입`"으로 지정할 수 있는 특성
> 
> ✵ **타입 계층 방향** == "**리턴 타입**의" **타입 계층 방향**

<br/>

⁜ `슈퍼 타입` 대신 "`서브 타입`" **파라미터** <br/> :
_"<u>더 **완화**한 `사전 조건`</u>"을 정의하는 것과 동일 <br/><br/>
→ 클라이언트 입장에서 **대체 가능** <br/>
→ "<u>메서드 **파라미터**</u>" : **"[반공변성](#반공변성--contravariance-)"** 필요_

> ✅ `파라미터 타입 반공변성 (Parameter Type Contravariance)` <br/>: _부모 클래스에서 구현된 메서드를 자식 클래스에서 **오버라이딩** 할 때_,<br/>
> <u>부모 클래스에서 사용하는 " **파라미터 타입**</u>의 `슈퍼타입`"으로 지정할 수 있는 특성
>
> ✵ **타입 계층 방향** **↔** "**리턴 타입**의" **타입 계층 방향**

✵ _Java는 파라미터 반공변성을 지원하지 않음(`@Override`에 의해 에러 & 없을 경우 : `Overloading` 으로 간주)_

<br/>
<br/>

### `함수 타입` & 서브 타이핑
- 객체지향 언어 : **"이름없는" 메서드** 정의 허용
  - `익명 함수`(_Anonymous Function_) 
  - `함수 리터럴`(_Function Literal_) 
  - `람다 표현식`(_Lambda Expression_)

<br/>

#### 〚 논리 〛
⭐️ " <u>파라미터 타입</u>이 **반공변성**을 가지고 <u>리턴 타입</u>이 **공변성**을 가진다. "<br/>
== " **<u>Override</u> 가 가능**하다. "<br/>
== " 메서드가 <u>**대체** 가능</u>하다. "<br/>
== " 두 메서드 사이에 <u>**서브 타이핑** 관계</u>가 존재한다. "

이에 따라 `new Customer().order( (publisher : Publisher) -> new Magazine(publisher) )` 와 같은 구조의 코드가 실행될 수 있다.
...
---
#### 공변성 ( _Covariance_ )
:`S`와 `T` 사이의 서브타입 관계가 **그대로 유지**된다.<br/>
(해당 위치에서 서브타입인 `S`가 슈퍼타입인 `T` 대신 사용될 수 있다. :: 관련 원칙 →`리스코프 치환 원칙`)

<br/>

#### 반공변성 ( _Contravariance_ )
:`S`와 `T` 사이의 서브타입 관계가 **역전**된다.<br/>
(해당 위치에서 슈퍼타입인 `T`가 서브타입인 `S` 대신 사용될 수 있다.)

<br/>

#### 무공변성 ( _Invariance_ )
:`S`와 `T` 사이의 **어떤한 관계도 없다**.<br/>
(`S` 대신 `T` / `T` 대신 `S` ❌)